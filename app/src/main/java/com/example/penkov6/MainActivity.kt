...
import com.example.penkov6.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.tvQuestion.text = "Первый вопрос"
        fillRounds()
        updateUI()
    ...
    private fun processRound(givenId: Int){
        if(checkAnswer(givenId)){
            if(!goNextRound()){
                Toast.makeText(this, "Поздравляю! Это победа", Toast.LENGTH_SHORT).show()
                finish()
            }
        } else{
